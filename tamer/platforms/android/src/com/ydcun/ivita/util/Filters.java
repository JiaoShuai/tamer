...
public class Filters{
	...
	*/
	public static double[] bandpassFilter(double[] list,int L,int minp,int maxp){
		int len=list.length;
		Complex[] theList = new Complex[L];
		for (int i = 0; i < len; i++) {
	...
	public static double[] sppsFilter(double[] list,int L,int K/*频谱排序的大小*/){
		int len=list.length;
		Complex[] theList = new Complex[L];
		for (int i = 0; i < len; i++) {
			theList[i] = new Complex(list[i], 0);
